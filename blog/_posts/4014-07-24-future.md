...
- Cancellation
  - Cooperative. Pass-down.
  - (Outer) Signalling cancellation: timers and manual.
  - (Outer) Detecting cancellation: OperationCanceledException and TaskCanceledException.
  - (Inner) Polling for cancellation (common in synchronous / CPU-based methods).
  - (Inner) Registering for cancellation (common in asynchronous / IO-based methods).
  - Linking cancellation tokens. The danger of multiple cancellation tokens and OperationCanceledException.CancellationToken from the Outer perspective.
