...
Here's a simple implementation using `async`-compatible synchronization primitives from [AsyncEx](https://github.com/StephenCleary/AsyncEx) (which supports all the same targets as Microsoft.Bcl.Async):
...
There is an implementation of this type in the [AsyncEx library](https://github.com/StephenCleary/AsyncEx) which is more complex than the simple version in this blog post. The AsyncEx version includes:
